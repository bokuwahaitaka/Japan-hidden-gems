-- Japan Hidden Gems: privacy-aware demographic and tag rankings
begin;
...
create policy "active_song_tags_public_read"
on public.song_tags for select to anon, authenticated
...
begin
  ...
  select
    ...
    s.is_hidden,
    s.created_at,
    (select count(*)::bigint from public.recommendations r where r.song_id = s.id),
    (select count(*)::bigint from public.ratings rt where rt.song_id = s.id),
    coalesce(
      (select jsonb_agg(a.tag_id order by a.tag_id) from public.song_tag_assignments a where a.song_id = s.id),
      '[]'::jsonb
    )
  from public.songs s
...
create or replace function public.admin_set_song_tags(
  p_song_id bigint,
  p_tag_ids bigint[]
)
returns void
...
begin
  ...
  if exists (
    select 1
    from unnest(coalesce(p_tag_ids, '{}'::bigint[])) as requested(tag_id)
    where not exists (
      select 1 from public.song_tags t
      where t.id = requested.tag_id and t.is_active
    )
  ) then
  ...
  select p_song_id, requested.tag_id, auth.uid()
  from unnest(coalesce(p_tag_ids, '{}'::bigint[])) as requested(tag_id);
end;
$$;
...
as $$
  with active_respondents as (
    ...
    where exists (select 1 from public.recommendations r where r.user_id = p.user_id)
       or exists (select 1 from public.ratings rt where rt.user_id = p.user_id)
  ),
  country_options as (
    select
      'country'::text,
...
begin
  ...
  if p_tag_id is not null and not exists (
    select 1 from public.song_tags
    where id = p_tag_id and is_active
  ) then
    raise exception 'Invalid tag.';
  end if;
